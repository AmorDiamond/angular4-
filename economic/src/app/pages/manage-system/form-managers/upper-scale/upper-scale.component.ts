...
@Component({
  selector: 'app-upper-scale',
  templateUrl: './upper-scale.component.html',
  styleUrls: ['./upper-scale.component.css']
})
export class UpperScaleComponent implements OnInit {
  ...
  nowPagesNum = 0;
  list = [];
  listParams = {url: '/manager/v1/epBaseInfoPojo/findListByCondition', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.upperScale;
    this.getList();
  }
