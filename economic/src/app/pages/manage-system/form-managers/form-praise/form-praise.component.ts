...
@Component({
  selector: 'app-form-praise',
  templateUrl: './form-praise.component.html',
  styleUrls: ['./form-praise.component.css']
})
export class FormPraiseComponent implements OnInit {
  ...
  nowPagesNum = 0;
  list = [];
  listParams = {url: '/manager/v1/eCIHonorPojo/getGOVERNMENTByCompany', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.formPraise;
    this.getList();
  }
  ...
  getList() {
    ...
    this.findListByUrl(this.getListParams, url).subscribe(res => {
      ...
      if (res.responseCode === '_200') {
        this.list = res.data.eCIHonorPojo;
