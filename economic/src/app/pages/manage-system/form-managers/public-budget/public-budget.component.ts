...
@Component({
  ...
  templateUrl: './public-budget.component.html',
  styleUrls: ['./public-budget.component.css']
})
export class PublicBudgetComponent implements OnInit {
  ...
  list = [];
  listParams = {url: '/manager/v1/macroPublicBudget/findByYearAndArea', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.publicBudget;
    this.getList();
  }
