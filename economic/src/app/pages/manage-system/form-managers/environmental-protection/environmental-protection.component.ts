...
@Component({
  selector: 'app-environmental-protection',
  templateUrl: './environmental-protection.component.html',
  styleUrls: ['./environmental-protection.component.css']
})
export class EnvironmentalProtectionComponent implements OnInit {
  ...
  list = [];
  listParams = {url: '/manager/v1/Environmental/findByNameAndYear', method: 'get'};
  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.environmentalProtection;
    this.getList();
  }
