...
@Component({
  selector: 'app-form-torch',
  templateUrl: './form-torch.component.html',
  styleUrls: ['./form-torch.component.css']
})
export class FormTorchComponent implements OnInit {
  ...
  nowPagesNum = 0;
  list = [];
  listParams = {url: '/manager/v1/eIIApAssets/findEIIApAssets', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.formTorch;
    this.getList();
  }
  getList() {
    ...
    this.findListByUrl(this.getListParams, url).subscribe(res => {
      ...
      if (res.responseCode === '_200') {
        this.list = res.data.eIIApAssetsPojo;
