...
@Component({
  selector: 'app-famous-excellent',
  templateUrl: './famous-excellent.component.html',
  styleUrls: ['./famous-excellent.component.css']
})
export class FamousExcellentComponent implements OnInit {
  ...
  nowPagesNum = 0;
  list = [];
  listParams = {url: '/manager/v1/eQIFamousProductPojo/getEQICertificationPojoByCompany', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.famousExcellent;
    this.getList();
  }
  ...
  getList() {
    ...
    this.findListByUrl(this.getListParams, url).subscribe(res => {
      ...
      if (res.responseCode === '_200') {
        this.list = res.data.eQIFamousProductPojo;
