...
@Component({
  selector: 'app-macro-economy',
  templateUrl: './macro-economy.component.html',
  styleUrls: ['./macro-economy.component.css']
})
export class MacroEconomyComponent implements OnInit {
  ...
  list = [];
  listParams = {url: '/manager/v1/macroGdp/getGDPByAreaAndYear', method: 'get'};

  formExcelTemplateData: any;
  ngOnInit() {
    const formExcelTemplateData = this.formManagersService.getFormExcelTemplateDownloadData();
    this.formExcelTemplateData = formExcelTemplateData.localtion + formExcelTemplateData.macroEconomy;
    this.getList();
  }
